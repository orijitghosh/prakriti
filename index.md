...
30 color palettes for R, each one pulled from an Indian landscape.
Himalayan ice. Thar dunes at sunset. Flamingos over the Rann.
Bioluminescent shores off Havelock. The autumn chinar canopy in Kashmir.

![prakriti logo animation](logo_video.gif)

Palettes come in three types (sequential, diverging, qualitative) and
plug straight into `ggplot2` with
...
They also work fine with base graphics, `lattice`, or anything else that
takes hex codes.
...
diverging palettes default to continuous. Override with
`discrete = TRUE/FALSE`.
...
| [`display_prakriti()`](https://orijitghosh.github.io/prakriti/reference/display_prakriti.md) | Plot one or all palettes |
| [`prakriti_names()`](https://orijitghosh.github.io/prakriti/reference/prakriti_names.md) | Character vector of palette names |
| [`prakriti_info()`](https://orijitghosh.github.io/prakriti/reference/prakriti_info.md) | Data frame with name, type, length, inspiration |
...
palette, open an issue with reference photos and suggested hex codes.
