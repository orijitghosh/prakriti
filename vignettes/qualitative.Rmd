---
title: "Recipes: qualitative palettes"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Recipes: qualitative palettes}
...
Qualitative palettes are for categorical data where no category is "higher"
or "lower" than another. The colors need to be distinct, not ordered. This
article shows common plot types where qualitative palettes shine.
...
Simulated flamingo flock sizes across months. Using `scale_fill_manual()`
with colors pulled from `prakriti_palette()` gives you full control when
you need the fill and color aesthetics to match.

```{r violin, fig.height = 4}
set.seed(123)
...
A 7-color showcase. `kutch_textile` has enough distinct colors for complex
categorical data.
...
6 vehicle classes, each getting a prayer-flag primary. `ladakh_monastery`
keeps groups separable even when points overlap.
...
`geom_hex()` for bivariate density.
